...
function Invoke-Ngen {
    ...
    if ($PSCmdlet.ParameterSetName -eq 'Directory') {
        ...
        $Rnd = Get-RandomAlphanumericString -Length 8
        $OutFile = Join-Path $Directory "ngen_outpuut_$Rnd_.txt"
        New-Item -Path $OutFile -ItemType File -Force | Out-Null

        Get-ChildItem -LiteralPath $Directory -Recurse:$Recurse | Foreach-Object {
            ...
            if ($_.Extension -match '\.(exe|dll)$') {
                ...
                if ($NoDependencies -or $Recurse) {
                    ngen install $_.FullName /NoDependencies /nologo | Out-File -FilePath $OutFile -Append
                } else {
                    ngen install $_.FullName /nologo | Out-File -FilePath $OutFile -Append
                }
            }
